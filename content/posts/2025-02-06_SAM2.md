...
In the self-attention block, \(I + \text{PE}(I)\) for the query and key, while \(I\) is passed as the value. This gave me question mark because normally, query, key, and value are the same, which is input feature added to positional embedding. But why they didn't add positional embedding for the value? 

<!-- The memory attention uses self-attention and cross-attention. If you are not familar with cross-attention, please refer to my [previous post](https://baampark.github.io/posts/2025-01-29_sam/#:~:text=This%20process%20is%20called%20cross%2Dattention.).  -->
