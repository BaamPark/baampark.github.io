...
\[
V^\pi(s) \triangleq \mathbb{E}_\pi \left[ G_t \mid S_t = s \right] = \mathbb{E}_\pi \left[ \sum_{k=0}^\infty \gamma^k R_{t+k} \mid S_t = s \right]
\]
...
\[
V^\pi(s) = \mathbb{E}_\pi \left[ R_t + \gamma G_{t+1} \mid S_t \right]
\]
...
The Q function \(Q^{\pi}(s,a)\) explicitly conditions on both state and action, which provides a more granular view of the agent's behavior and allows for better decision-making. We can write relationship between value function \(V^{\pi}\) and Q function \(Q^\pi\).
